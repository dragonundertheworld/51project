...
MOV A,SBUF
;接受主机发来的地址信息
CJNE A, #0,CHECK ;判断发送的地址是否 与本机地址相等，address(这里是1)表示本机地址
;若不相等则继续等待，不做处理
...
ACK0:
SETB TB8
MOV SBUF,#0;向主机发送应答信号，即发回本机地址(1)给主机
JNB TI, $;trasmission
;判断发送缓冲区是否为空?
CLR TI
;清零发送中断标志II
...
MOV A,SBUF
;接受主机发来的地址信息
CJNE A, #0,CHECK1 ;判断发送的地址是否 与本机地址相等，address(这里是1)表示本机地址
;若不相等则继续等待，不做处理
CLR SM2
;将SM2位清零
;从机应答
...
MOV A,SBUF
;接受主机发来的地址信息
CJNE A, #0,CHECK2 ;判断发送的地址是否 与本机地址相等，address(这里是1)表示本机地址
;若不相等则继续等待，不做处理
CLR SM2
;将SM2位清零
;从机应答
